...
public class LedManagerService extends ILedManager.Stub {
    ...
    */
    ...
    public void removeEvent(int appId, IBinder token) {
        synchronized(mLock) {
            if(mCurrentLedInfo != null && mCurrentLedInfo.getAppId() == appId) {
                if(DEBUG_SERVICE) Log.d(TAG, "removeEvent: remove current");
                turnOff();
            }
            mLedInfoQueue.dequeueLedInfoLocked(appId);
            mLedInfoQueue.scheduleLedInfoLocked();
        }
    }
    ...
    class LedManagerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if(DEBUG_SERVICE) Log.d(TAG, "handleMessage: msg: " + msg.what + ", info: " + (LedInfo)msg.obj);
            switch (msg.what) {
                case TURN_OFF_MSG:
                    turnOff();
                    break;
    ...
    private void setFinishEventLocked() {
        ...
        setLedLocked(0, 0, 0, 0);
        mCurrentLedInfo = null;
    }

    private void setLedLocked(int led1, int onMs, int offMs, int option) {
        setLed_native(mNativePointer, led1, onMs, offMs, option);
    }
    ...
    private static native void setLed_native(int ptr, int led1, int onMS, int offMS, int option);
