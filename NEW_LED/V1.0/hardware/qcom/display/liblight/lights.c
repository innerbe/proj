...
write_int(char const* path, int value)
{
    ...
    if(DEBUG && (strstr(path, "backlight") == NULL))
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "write_int() path:%s, value:0x%02x, fd:%d", path, value, fd); 
#endif
    if (fd >= 0) {
        char buffer[20];
...
{
    if(DEBUG)
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "------------begin set_light_oem_led-----------\n");
    pthread_mutex_lock(&g_lock);

    if(DEBUG)
    __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "led1 : 0x%08x, onMs: %d, offMs: %d, option: %d", state->led1, state->flashOnMS, state->flashOffMS, state->option);
    ...
    if(option > 0) {
        // pre-defined pattern
        //write_int(PATTERN_FILE, option);
    } else {
        write_int(RED_LED_FILE, red);
        write_int(GREEN_LED_FILE, green);
        write_int(BLUE_LED_FILE, blue);
        // write_int(ON_MS_FILE, onMs);
        // write_int(OFF_MS_FILE, offMs);
    }

    if(DEBUG)
        __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "------------end. set_light_oem_led-----------\n");
    pthread_mutex_unlock(&g_lock);
    return 0;
}
#endif
...
//+US1-CF1
        ...
        if(DEBUG)
            __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, "open_lights name:%s", name);
